...
<div id="main"  > 
{mode ==='step' && <h2 style={{textAlign:'center',color:'whitesmoke'}}>Korak {currrentStep}/{map.length}</h2>  }

{map.map(([x,y],id)=> <Coin key={id} number={id} x={x} y={y} collected={collected}  />   )}
<Agent  name={agent} x={agentPosition.x}  y={agentPosition.y }   />
{ pause==false && currentStepRef.current>0 && currentStepRef.current<path.length-1 && mode=="auto" && gameOver.length==0 ?<h1  style={{textAlign:'center',color:'red'}}>Pauza</h1>:<h1></h1>}
...
<p>===============</p>
<p style={{display:'flex', justifyContent:'center',alignItems:'center'}}  >Ukupna cena: {stepsTaken.reduce((sum, { price }) => sum + price, 0)}</p>
</div>
</div>
